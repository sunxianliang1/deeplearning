...
class realplay():
    ...
    def play(self):
        with open(self.path+'\capacitance data int.txt','r') as f:
    ...
    def play2(self):
        ...
        a=self.lines[618].split(' ',1)
        strmeasure=a[1].split()
        intmeasure=list(map(int,strmeasure))
        measure=np.asarray(intmeasure)
        print (a[0])

        de=(measure-self.intempty).reshape(1,-1)
